...
public:
    ...
    void sort(Array<T>& array) override
    {
        ...
        // ћассив уже отсортирован
        if (min == max)
        {
            return;
        }

        // ѕромежуточный массив
        Array<T> aux = Array<T>(std::round(array.size() * FACTOR));
        aux.fill(NULL);

        //  остыль дл€ возможности сортировки большего диапазона чисел
        // —читаем количество нулей в исходном массиве и
        ...
        for (std::size_t index = 0; index < array.size(); index++)
        {
            ...
            // —читаем количество отрицательных чисел,
            // дл€ определни€ позиции добавлени€ нулей
            if (array[index] < 0)
            {
                negativeCount++;
            }

            // ¬ысчитывание hash-index'а 
            std::size_t hashIndex = (array[index] - min) * (aux.size() - 1) / (max - min);

            // ≈сли образовалась коллизи€
            if (aux[hashIndex] != NULL)
            {
                ...
                // ≈сли "окно" найти не удалось, то начинаетс€ поиск ближайшего
